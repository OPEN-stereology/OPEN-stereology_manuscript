% ### Attribution Notice 
% 
% This MATLAB class `CameraController.m` originally created as a controller for 
% [digiCamControl](http://digicamcontrol.com/), a free and open-source DSLR control snippet for Windows.
% Not authored by Open-Stereo project, but used as permitted by the
% original source - distributed via forums and user communities.
...
%Update ReadMe:
% fprintf(fopen('ReadMe.txt','w'),'%s',regexprep(help('CameraController'),'<a .*?>|</a>',''));fclose all
...
% http://localhost:5513/out.jpg                                        %live preview current frame
...
classdef YawCameraController < handle
    %% Properties
    ...
    methods
        ...
        function [I,err] = LiveView(C)
            ...
            if strcmp(C.connection,'HTTP')
                try
                    I = imread(['http://' C.dcc ':5513/out.jpg'],'jpg');
                catch e
